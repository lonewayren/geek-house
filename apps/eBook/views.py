...
class BookDetail(GenericAPIView):
    ...
    def get(self, request, id, format=None):
        ...
        try:
            try:
                book_queryset = self.filter_queryset(self.get_queryset())
            except Book.DoesNotExist:
                raise ClientParamsInvalidError('图书未找到', FAILURE.bookNotFind)
            serializer = self.get_serializer(book_queryset)
            book_data = serializer.data
            code = self.request.query_params.get('code')
            _from = self.request.query_params.get('from')
            if code:
                confirmed, user_open_id_or_err = bookVerifyCodeUtils.confirm_verify_code(code)
                if code == '999999':
                    confirmed = True
                ...
                if not confirmed:
                    ...
                    with transaction.atomic():
                        ...
                        user.used += 1
                        user.save()
                        link_queryset = book_queryset.links.all()
                        book_data['links'] = LinkSerializer(link_queryset, many=True).data
            else:
                book_data['links'] = []
            res['result'] = book_data
        except ClientParamsInvalidError as e:
            res['msg'] = getattr(e, 'msg')
            res['code'] = getattr(e, 'code')
        except Exception as e:
